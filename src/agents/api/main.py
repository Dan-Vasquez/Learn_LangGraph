...
def chat(chat_id: str, item: Message):
    response = support_agent.invoke({"messages": [{"role": "user", "content": item.message}]})
    return {"response": response.get("messages")[-1].text}

@app.post("/chat_db/{chat_id}")
async def chat(chat_id: str, item: Message, checkpointer: CheckpointerDep):
    ...
    support_agent = make_graph(config={"checkpointer": checkpointer})
    state = {"messages": [human_message]}
    response = support_agent.invoke(state, config)
    last_message = response["messages"][-1]
    # CRUD add message
    return response["messages"]


@app.post("/chat2/{chat_id}/stream")
async def stream_chat(chat_id: str, message: Message, checkpointer: CheckpointerDep):
    ...
    async def generate_response():
        support_agent = make_graph(config={"checkpointer": checkpointer})
        for message_chunk, metadata in support_agent.stream({"messages": [human_message]}, stream_mode="messages"):
