...
class PerformanceReview(BaseModel):
    bottlenecks: list[str] = Field(description="The performance bottlenecks in the code", default=None)
    efficiencyScore: int = Field(description="The efficiency score of the code from 1 to 10", default=None, ge=1, le=10)
    optimizationSuggestions: list[str] = Field(description="The optimization suggestions for improving the code", default=None)
    recommendations: list[str] = Field(description="The recommendations for improving the code", default=None)
...
def performance_review(state: State):
    code = state['code']
    messages = [
        ("system", "You are an expert in code performance. Focus on identifying performance bottlenecks, inefficient algorithms, and resource usage."),
        ("user", f"Review this code: {code}")
    ]
